...
window.onload = function initTriangles(){
    ...
    // Setup event listeners for mouse drag to rotate  
	canvas.addEventListener('mousedown', startRotate, false);  
	canvas.addEventListener('mouseup', stopRotate, false);  
...
function divideTriangle( a, b, c, count ){
 ...
 function rotateTrianglePoints(angleX, angleY) {  
            ...
            for (let i = 0; i < points.length; i += 3) {  
                const vec = vec3.fromValues(points[i], points[i + 1], points[i + 2]);  
                const transformed = vec3.create();  
                vec3.transformMat4(transformed, vec, rotationMatrix);  
                transformedPoints.push(transformed[0], transformed[1], transformed[2]);  
            }  
...
function renderTriangles(){
 ...
 function startRotate(e) {  
        ...
        function rotate(e) {  
            ...
            const dx = e.clientX - canvas.offsetLeft - lastX;  
            const dy = e.clientY - canvas.offsetTop - lastY;  
            ...
            renderTriangles(); // Re-render after rotation  
        }  
